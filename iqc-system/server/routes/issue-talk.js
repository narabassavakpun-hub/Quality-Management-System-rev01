...
router.post('/', auth, uploads.issueTalk.array('files', 10), uploads.verifyMagic, uploads.compressImages, uploads.compressVideo, (req, res) => {
...
router.post('/:id/messages', auth, uploads.issueTalk.array('files', 10), uploads.verifyMagic, uploads.compressImages, uploads.compressVideo, (req, res) => {
  const access = getAccess(+req.params.id, req.user.id);
  if (!access) return res.status(403).json({ error: 'ไม่มีสิทธิ์เข้าถึง' });
