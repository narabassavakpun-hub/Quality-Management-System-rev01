...
// คลัง (หัวหน้าคลัง) เป็นผู้กด acknowledge แทน qc_staff/qc_supervisor เดิม — ไม่มี default seed user
// สำหรับ role นี้ (เหมือน purchasing_manager) จึงสร้าง fixture user ตรงๆ ในเทสต์นี้
...
const C = {};
for (const [k, un] of [['pur', 'purchasing1'], ['staff', 'qc_staff1'], ['sup', 'supervisor1'], ['wh', 'warehouse_test1']]) { setSess(un, k); C[k] = 'token=' + jwt.sign({ id: uid(un), sessionToken: k }, process.env.JWT_SECRET); }
...
test('DEL-05 warehouse acknowledge pending → acknowledged', async () => {
  const r = await api('POST', `/api/delivery/${schedId}/acknowledge`, { cookie: C.wh });
  assert.equal(r.status, 200);
  assert.equal(r.body.status, 'acknowledged');
});

test('DEL-05b permission: qc_staff can no longer acknowledge (moved to warehouse) → 403', async () => {
  const id = await mkPending();
  const r = await api('POST', `/api/delivery/${id}/acknowledge`, { cookie: C.staff });
  assert.equal(r.status, 403);
});
