...
// ดู Session bandwidth audit รอบ 2) — UPLOADS_BASE/SYNC_STATE_PATH เป็น path จริงของ repo (hardcode ใน
// backupService.js ไม่ผ่าน env var เหมือน IQC_DB_PATH) จึงต้องสร้าง/ลบไฟล์ทดสอบใน iqc-system/uploads/ จริง
// แล้วล้าง state ที่เขียนไว้ให้สะอาดหลังจบเทสเสมอ (ไม่ทิ้ง key แปลกปลอมไว้ใน upload-sync-state.json ของ repo)
const REAL_UPLOADS_BASE = path.join(__dirname, '../../uploads');
const REAL_SYNC_STATE_PATH = path.join(REAL_UPLOADS_BASE, '..', 'upload-sync-state.json');
