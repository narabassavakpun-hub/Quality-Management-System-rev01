...
// supplier ที่ตัวเอง assign ไว้จริงเท่านั้น — เข้มงวด ไม่รวม supplier ที่ยังไม่มีผู้ดูแล ต่างจาก NCR/UAI/Delivery
// action-permission ที่มี fallback รวมด้วย ดู purchasingScope.js — bug จริงที่ user รายงาน: "ผู้ผลิตของฉัน" เคยโชว์
// supplier ทุกรายที่ยังไม่มีผู้ดูแลปนมาด้วย ไม่ตรงกับที่ตั้งค่าไว้ใน Master List), bucket mapping ถูกต้อง, pagination/filter
...
test('DASH-02 summary: purchasing1 เห็นเฉพาะ supAssigned (ไม่รวม supOther, ไม่รวม supOpen ที่ยังไม่มีผู้ดูแล)', async () => {
  const r = await api('GET', '/api/purchasing/dashboard/summary', { cookie: C.pur1 });
  assert.equal(r.status, 200);
  assert.equal(r.body.supplier_count, 1); // supAssigned เท่านั้น
  assert.equal(r.body.ncr_waiting_review, 1); // supAssigned เท่านั้น (ไม่รวม supOpen)
...
test('DASH-03 summary: purchasing2 เห็นเฉพาะ supOther (ไม่รวม supOpen)', async () => {
  const r = await api('GET', '/api/purchasing/dashboard/summary', { cookie: C.pur2 });
  assert.equal(r.status, 200);
  assert.equal(r.body.supplier_count, 1); // supOther เท่านั้น
  assert.equal(r.body.ncr_waiting_review, 1); // supOther เท่านั้น
});
...
test('DASH-07 ncrs list: purchasing1 filter bucket=waiting_review เห็นเฉพาะของ supAssigned (ไม่เห็นของ supOther หรือ supOpen)', async () => {
  const r = await api('GET', '/api/purchasing/dashboard/ncrs?bucket=waiting_review&limit=50', { cookie: C.pur1 });
  assert.equal(r.status, 200);
  assert.equal(r.body.total, 1);
  assert.equal(r.body.data[0].supplier_id, supAssigned);
});
