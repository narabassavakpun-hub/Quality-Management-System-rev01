...
const result = db.prepare(
  "UPDATE sqlite_master SET sql = REPLACE(sql, '\"ncrs_old\"', 'ncrs') WHERE sql LIKE '%ncrs_old%' AND type IN ('table','index','trigger','view')"
).run();
db.pragma('writable_schema = OFF');
console.log(`Fixed ${result.changes} object(s) in sqlite_master`);
