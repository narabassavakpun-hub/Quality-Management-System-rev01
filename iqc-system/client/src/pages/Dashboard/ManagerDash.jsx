...
export default function ManagerDash({ navigate }) {
  const { data } = useStats();
  const openNCR = data?.ncr_open_recent || [];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <SummaryCard value={data?.uai_not_final_count ?? 0} label="UAI รอดำเนินการ" color="warning" />
        <SummaryCard value={data?.ncr_pending_supplier_count ?? 0} label="รอตรวจ Supplier" color="accent" />
      </div>
      <div className="card">
