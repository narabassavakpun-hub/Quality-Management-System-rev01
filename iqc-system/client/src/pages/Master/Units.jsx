...
function Form({ initial = {}, onSave, loading, error }) {
  const [form, setForm] = useState({ name: '', abbreviation: '', ...initial });
  const set = (k, v) => setForm(p => ({ ...p, [k]: v }));
  return (
    <form onSubmit={e => { e.preventDefault(); onSave(form); }} className="space-y-3">
      {error && <div className="text-danger text-small">{error}</div>}
      <div><label className="label">ชื่อหน่วยนับ *</label><input className="input" value={form.name} onChange={e => set('name', e.target.value)} required /></div>
      <div><label className="label">ตัวย่อ (เช่น pcs, m, kg)</label><input className="input" value={form.abbreviation} onChange={e => set('abbreviation', e.target.value)} /></div>
...
export default function Units() {
  ...
  const { data: resp = {} } = useQuery({
    queryKey: ['units', showAll, page, debouncedSearch],
    queryFn: () => api.get('/master/units', { params: { page, limit: PAGE_SIZE, q: debouncedSearch, ...(showAll ? { all: '1' } : {}) } }).then(r => r.data),
  });
  ...
  const save = useMutation({
    mutationFn: (form) => editing ? api.patch(`/master/units/${editing.id}`, form) : api.post('/master/units', form),
    onSuccess: () => { qc.invalidateQueries(['units']); setModalOpen(false); setEditing(null); },
  });
  const toggle = useMutation({
    mutationFn: (id) => api.patch(`/master/units/${id}/toggle`),
    onSuccess: () => { qc.invalidateQueries(['units']); setConfirmToggle(null); },
  });
  ...
  return (
    <div>
      <h1 className="text-h2 font-bold text-text mb-4">หน่วยนับ</h1>
      ...
      <div className="table-container">
        <table className="table">
          <thead><tr>
            ...
            <SortTh col="abbreviation" sortKey={sortKey} sortDir={sortDir} onSort={onSort}>ตัวย่อ</SortTh>
          ...
          <tbody>
            ...
            {rows.length === 0 && <tr><td colSpan={4} className="text-center text-muted py-8">ไม่พบข้อมูล</td></tr>}
          </tbody>
        </table>
      </div>
      <Pagination page={page} totalPages={totalPages} total={totalRows} limit={PAGE_SIZE} onChange={setPage} />
      <Modal open={modalOpen} onClose={() => { setModalOpen(false); setEditing(null); }} title={editing ? 'แก้ไขหน่วยนับ' : 'เพิ่มหน่วยนับ'}>
      ...
      <ExcelImportModal
        ...
        title="Import หน่วยนับ" apiPath="/master/units"
        onDone={() => qc.invalidateQueries(['units'])}
      />
    </div>
  );
}
