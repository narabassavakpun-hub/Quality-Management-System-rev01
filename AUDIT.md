...
| D5 | ~~ไม่มี online backup อัตโนมัติ~~ → **มีแล้วจริง (พบใหม่ตอนตรวจก่อนทำ deploy redesign)** | ✅/🟡 | **Finding นี้ล้าสมัย** — โค้ดจริงมี automated backup อยู่แล้ว (`server/lib/backupService.js` + `bootstrap.js` + `restoreService.js` + `r2Client.js`, ไม่ได้บันทึกไว้ใน DEVLOG ตอนสร้างจึงตกหล่นตอน audit รอบนี้): online `VACUUM INTO` + `PRAGMA quick_check` verified snapshot ทุก ~10 นาที (`backups/db/latest.db`) + daily FIFO 7 slot (`backups/db/day-N.db`, Bangkok weekday) + incremental uploads sync ไป Cloudflare R2, เรียกจาก scheduler ใน `index.js` (`setInterval` ทุก 10 นาที) และตอน graceful shutdown (`runHotBackup()` ก่อน `db.close()`) — ดู CLAUDE.md §27 / DEPLOYMENT.md §8.2 **เหลือ 🟡 จริงเฉพาะ**: ยังไม่มี restore-drill อัตโนมัติใน CI (กู้จริงทดสอบผ่าน `node scripts/restore-from-r2.js` แบบ manual + unit test เท่านั้น ไม่ได้รันอัตโนมัติทุก deploy) |
